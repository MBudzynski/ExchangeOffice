package pl.sda.exchangeoffice.external.exchangerate;
...
import pl.sda.exchangeoffice.domain.curencyrate.AllCurrencyRateResponse;
...
public class ExchangeRateClient {
    ...
    public CurrencyRateResponse findCurrencyRatefromlast10days(String currency, LocalDate formDate, LocalDate toDate) throws JsonProcessingException {
        String address = String.format("%s/exchangerates/rates/a/%s/%s/%s/?format=json",
                nbpProperties.getUrl(), currency, formDate.toString(), toDate.toString());
        ...
        if (rateRsp.getStatusCodeValue() == 404) {
            return findCurrencyRate(currency, formDate.minusDays(1));
        }

        throw new IllegalStateException("It was impossible to fetch currency rate " + rateRsp.getStatusCodeValue());
    }

    public AllCurrencyRateResponse[] findAllCurrencyRate() throws JsonProcessingException {
        String address = String.format("%s/exchangerates/tables/a/?format=json",
                nbpProperties.getUrl());

        ResponseEntity<String> rateRsp =
                restTemplate.exchange(address, HttpMethod.GET, HttpEntity.EMPTY, String.class);

        if (rateRsp.getStatusCode().is2xxSuccessful()) {
            return objectMapper.readValue(rateRsp.getBody(), AllCurrencyRateResponse[].class);
        }
